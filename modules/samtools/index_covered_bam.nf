process INDEX_FILTERED_BAM {
  ...
  publishDir "${params.sample_folder}/bams", pattern: "*_covered.bai", mode:'copy'
  ...
  tuple val(sampleID), file("*_covered.bai"), emit: covered_bai

  script:
  log.info "----- Index Bam for Sample: ${sampleID} -----"

  """
  samtools index -b ${bam} > ${sampleID}_covered.bai
  """
}