...
physical_1M_grid <- Reduce(rbind,purrr::transpose(grids)[[1]])
genetic_1M_grid <- Reduce(rbind,purrr::transpose(grids)[[2]])

# write the files
write.csv(physical_1M_grid, file.path(quilt_dir,"data/quilt_1M_physical_grid.csv"), row.names = F, quote = F)
write.csv(genetic_1M_grid, file.path(quilt_dir,"data/quilt_1M_genetic_grid.csv"), row.names = F, quote = F)
